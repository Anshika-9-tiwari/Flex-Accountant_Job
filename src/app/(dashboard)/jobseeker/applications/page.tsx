// src/app/(dashboard)/jobseeker/applications/page.tsx
...
function formatSalary(application: JobseekerApplication) {
  const { salaryMin, salaryMax, salaryType } = application.job;
  ...
  const filteredApplications = useMemo(() => {
    return applications.filter((application) => {
      ...
      const matchesSearch =
        ...
        application.job.location.toLowerCase().includes(keyword);
        ...
        <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-base leading-7 text-white/85 sm:text-lg">
              View the jobs you applied to and track their application status
              from one place.
            </p>
          </div>
      ...
      {!loading && !error && filteredApplications.length === 0 && (
        <div className="rounded-2xl bg-white p-10 text-center shadow-sm">
          ...
          <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-slate-500">
            You have not applied to any jobs yet, or no applications match your
            current filters.
          </p>
